  ...
  def create
    ...
    if params['vmguestip']
      if api.is_secondary_ip(@resource[:virtual_machine_id], params['vmguestip'])
        ...
        raise "Secondary IP #{params['vmguestip']} is currently owned by #{vm['displayname']}. You can: a) reprovision #{vm['displayname']} OR b) move your secondary ip to a different last octet..." if vm['
